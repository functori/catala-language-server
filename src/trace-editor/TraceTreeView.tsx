...
function subtreeHasMismatch(
  ...
  const newPrefix = (c : TraceElement) => {
    if ((c.element.kind === 'scope_call' || c.element.kind === 'scope_var' || c.element.kind === 'local_var') && typeof c.element.name === 'string') {
      if (childPrefix) {
  ...
  if (
    (el.element.kind === 'scope_call' || el.element.kind === 'scope_var' || el.element.kind === 'local_var') &&
    el.trace !== undefined
  ) {
...
export default function TraceTreeView({
  ...
  if (test !== undefined) {
    const [, outputs] = traceVariablesForTest(
      trace,
...
function TraceNode({
  ...
  if (
    (te2.element.kind === 'scope_call' || te2.element.kind === 'scope_var' || te2.element.kind === 'local_var') &&
  ...
  const hasMismatch =
    expected !== null &&
    subtreeHasMismatch(te2, childPrefix, expected);
  useEffect(() => {
    if (hasMismatch) {
      setExpanded(true);
    }
