...
function traceVariablesAux(
  ...
  for (const te of trace) {
    const k = te.element;
    const varCond = (k.kind === 'scope_var' && k.input !== 'only_input') ||
      k.kind === 'local_var'
    if (
      (k.kind === 'scope_call' || varCond) && te.trace !== undefined &&
      ...
      if (variables.length===0 && te.value !== undefined) {
        ...
        if (varCond && typeof k.name === 'string' && te.value !== undefined && (te.value.kind !== 'struct' && te.value.kind !== 'array')) {
